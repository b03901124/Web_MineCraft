...
Player.prototype.onMouseEvent = function( x, y, type, rmb )
{
	if ( type == MOUSE.DOWN ) {
		...
		console.log('length: ' + Math.abs( this.dragStart.x - x ) + Math.abs( this.dragStart.y - y ));
...
Player.prototype.doBlockAction = function( x, y, destroy )
{
	...
	var picklen = 6;
	var block = this.canvas.renderer.pickAt( new Vector( bPos.x - picklen, bPos.y - picklen, bPos.z - picklen ), new Vector( bPos.x + picklen, bPos.y + picklen, bPos.z + picklen ), x, y );
	console.log("destroy: " + destroy)
	console.log("block pos: ( " + block.x + ", " + block.y + ", " + block.z + ")")
	console.log("pos: ( " + (block.x + block.n.x) + ", " + (block.y + block.n.y) + ", " + (block.z + block.n.z) + ")")
	console.log("mat: " + this.buildMaterial.id)
	if ( block != false )
	{
