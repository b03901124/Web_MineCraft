...
Client.prototype.connect = function( uri, nickname )
{
	...
	socket.on( "worldID", function( data ) { s.onPlayerChangeWorldID( data ); } );
	socket.on( "setworld", function( data ) { s.onSetWorld( data ); } );
	socket.on( "tmpleave", function( data ) { s.onTmpPlayerLeave( data ); } );
	socket.on( "tmpjoin", function( data ) { s.onTmpPlayerJoin( data ); } );
	// socket.on( "newspawn", function() { s.onSetWorld(); } );
}
...
{
	// console.log(this.world.localPlayer);
	// this.socket.emit("debug", this.world.localPlayer)
...
// onPlayerChangeWorldID( data )
//
// Called when the server wants to change the player's world ID.

Client.prototype.onPlayerChangeWorldID = function( data )
{
	this.worldID = data.target;
}

// onSetWorld( data )
//
// Called when the server wants to change the player's world.

Client.prototype.onSetWorld = function( data )
{
	var world = this.world = new World( data.sx, data.sy, data.sz );
	world.createFlatWorld( data.sz );
	world.createFromString( data.blocks );

	if ( this.eventHandlers["setWorld"] ) this.eventHandlers.setWorld( world );
}

// onTmpPlayerLeave( data )
//
// Called when a player has temporarily left the game.

Client.prototype.onTmpPlayerLeave = function( data )
{
	if ( this.worldID == data.world ) {
		if ( this.world.players[data.nick].nametag ) {
			this.world.renderer.gl.deleteBuffer( this.world.players[data.nick].nametag.model );
...
// onTmpPlayerJoin( data )
//
// Called when a new player temporarily joins the game.

Client.prototype.onTmpPlayerJoin = function( data )
{
	if ( this.worldID == data.world ) {
		data.moving = false;
		data.aniframe = 0;
...
Server.prototype.setWorld = function( worlds )
{
	this.worlds = worlds;
}
...
Server.prototype.findPlayerByName = function( name )
{
	for ( var w in this.worlds ) {
		for ( var p in this.worlds[w].players ){
			if ( p.toLowerCase().indexOf( name.toLowerCase() ) != -1 ) return this.worlds[w].players[p];
		}
	}
	return null;
}

// changeWorld( socket, original, target )
//
// move player from original world to new world.

Server.prototype.changeWorld = function( socket, original, target )
{
	if ( this.log ) this.log( "Client " + this.getIp(socket) + " move from " + original + " to " + target );

    if ( socket._nickname != null )
    {
		delete this.worlds[original].players[socket._nickname];
		var world = this.worlds[target];
		// this.log(this.worlds[original].players)
		socket.emit( "worldID", { target: target } );
		socket.emit( "setworld", {
            sx: world.sx,
            sy: world.sy,
            sz: world.sz,
			blocks: world.toNetworkString() } );

		socket.broadcast.emit( "tmpleave", {
			world: original,
		...
		world.players[socket._nickname] = {
            socket: socket,
			worldID: target,
            nick: socket._nickname,
...
// changeWorldID( socket, target )
//
// change player's world ID.

// Server.prototype.changeWorldID = function( socket, target )
// {
// 	socket.emit( "worldID", { target: target } );
// }
...
Server.prototype.onConnection = function( socket )
{
	...
	socket.on( "debug", function(data) { s.log(data) } );
}
...
{
	var world = this.worlds[data.world];
	...
	this.log(Math.sqrt( (data.x-world.spawnPoint.x)*(data.x-world.spawnPoint.x) + (data.y-world.spawnPoint.y)*(data.y-world.spawnPoint.y) + (data.z-world.spawnPoint.z)*(data.z-world.spawnPoint.z)))
	var null_len = 3;
	if ( Math.sqrt( (data.x-world.spawnPoint.x)*(data.x-world.spawnPoint.x) + (data.y-world.spawnPoint.y)*(data.y-world.spawnPoint.y) + (data.z-world.spawnPoint.z)*(data.z-world.spawnPoint.z)  ) < null_len ) return false;
    ...
    if ( socket._nickname != null  )
    {
        try {
            ...
            var pl = world.players[socket._nickname];
            pl.blocks++;
...
Server.prototype.onPlayerUpdate = function( socket, data )
{
    ...
    if ( socket._nickname != null  )
    {
		for ( w in this.worlds ) {
			world = this.worlds[w];
			// this.log(w + " " + Object.getOwnPropertyNames(world.players))
			var pl = world.players[socket._nickname];
...
Server.prototype.onDisconnect = function( socket )
{
    ...
    if ( socket._nickname != null )
    {
        ...
        delete this.worlds["public"].players[socket._nickname];
